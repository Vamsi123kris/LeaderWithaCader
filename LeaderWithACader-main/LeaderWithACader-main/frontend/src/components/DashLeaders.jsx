...
export default function DashLeaders() {
  ...
  useEffect(()=>{
    const fetchPosts = async()=>{
      try {
        const res = await fetch(`/api/leader/getlead`)
...
const handleDeleteMla = async()=>{
  try {
     const res = await fetch(`/api/leader/delete?leadId=${mlaIdtoDelete}`,
       {
         method:'DELETE',
       }
     );
    ...
    dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
     ...
     <Table hoverable className="shadow-md">
      <Table.Head>
        ...
        <Table.HeadCell> partyName</Table.HeadCell>
      ...
      {mla.map((ml)=>(
        <Table.Body className="divide-y">
          <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
            ...
            <Table.Cell>{ml.designation}</Table.Cell>
            <Table.Cell>
            ...
            <Table.Cell>
                ...
                <Link to={`/update-lead/${ml._id}`} className="text-teal-500 hover:underline" >Edit</Link>
      ...
      <Modal.Body>
        <div className='text-center'>
          ...
          <h3 className='mb-5 text-lg text-gray-500 dark:text-gray-400'>
            Are you sure you want to delete leader?
          </h3>
